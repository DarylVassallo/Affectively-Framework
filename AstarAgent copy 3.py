...
def search(time_limit_count, actions_data, env, pos_pool, best_pos, furthest_pos, current_starting_pos_x, latest_save_num, visited_states, require_replanning, original_save_num, original_dist_x, original_dist_y, original_damage, original_death, original_score, original_kill_count):
    ...
    while best_pos.reached_end_count == 0 and search_count <= 500 and time_limit_count <= 350 and (len(pos_pool) != 0 and (((best_pos.pos_x - current_starting_pos_x) < max_right) or not current_good) and env.episode_length < 600):        
...
class AstarAgent:    
    ...
    def AStarRun(self, segments) :
        ...
        while True:   
            ...
            if main_search_count >= 500 or main_search_count == 0 or main_time_limit_count >= 350:
                playable = False
                print("Close 1")
                main_env.env.close()
