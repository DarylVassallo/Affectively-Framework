...
class GANLevelEnv(gym.Env):
    def __init__(self, gan_model=GANWrapper(), level_dim=32):
        ...
        self.action_space = spaces.Box(low=-1, high=1, shape=(self.level_dim * 2,), dtype=np.float32)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        while len(self.action_list) <= 0:
            ...
            action_1 = action[:self.level_dim]
    ...
    def step(self, action):
        ...
        score, enemy_count, arousal_left_count, arousal_middle_count, arousal_right_count, enemy_left_count, enemy_middle_count, enemy_right_count = self.reward(self.arousal_counter, self.current_segment, self.segment_count)

        print("STEP, PLAYABLE: " + str(self.playable) + ", AROUSALS: (" + str(arousal_left_count) + " : " + str(arousal_middle_count) + " : " + str(arousal_right_count) + "), SCORE: " + str(score))
        print("==========")
        self.score_count += score

        self.total_arousal_count += self.arousal_counter
        self.left_arousal_count += arousal_left_count
        self.middle_arousal_count += arousal_middle_count
        self.right_arousal_count += arousal_right_count 

        self.total_enemy_count += enemy_count 
        self.left_enemy_count += enemy_left_count
        self.middle_enemy_count += enemy_middle_count
        self.right_enemy_count += enemy_right_count  
    ...
    def reward(self, arousal_counter, segment, index): 
        ...
        # Choose the reward strategy
        reward = self.maximum_arousal_reward(arousal_counter)
        # reward = self.minimum_arousal_reward(arousal_counter)
        # reward, arousal_left, arousal_middle, arousal_right = self.multiple_arousal_reward(index, arousal_counter)
        # reward = self.maximum_enemy_reward(num_enemies)
        # reward = self.minimum_enemy_reward(num_enemies)
        # reward, enemy_left, enemy_middle, enemy_right = self.multiple_enemy_reward(index, num_enemies)  
        # Choose the reward strategy 
        
        return reward, num_enemies, arousal_left, arousal_middle, arousal_right, enemy_left, enemy_middle, enemy_right
    
    def maximum_enemy_reward(self, num_enemies):
        reward = num_enemies
        return reward
    
    def minimum_enemy_reward(self, num_enemies):
        reward = -num_enemies
        return reward
    
    def multiple_enemy_reward(self, index, num_enemies):
        left = 0
        middle = 0
        right = 0
    ...
    def maximum_arousal_reward(self, arousal_counter):
        reward = arousal_counter
        return reward
    
    def minimum_arousal_reward(self, arousal_counter):
        reward = -arousal_counter
        return reward
    
    def multiple_arousal_reward(self, index, arousal_counter):
        left = 0
        middle = 0
        right = 0
